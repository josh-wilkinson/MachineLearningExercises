...
def description_preprocessing(df):
    # this function is a modified version from the https://doi.org/10.1017/S1351324923000360

    # merge the neighborhood_overview and description columns into description
    df["neighborhood_overview"] = df["neighborhood_overview"].fillna("")
    df["description"] = df["description"].fillna("")
    df["description"] = df["name"] + " " + df["description"] + " " + df["neighborhood_overview"]
    df.drop(columns=["name", "neighborhood_overview"], inplace=True)  
    ...
    #convert 1,000 to 1000 and alike
    ...
    ''''''
    df["description"]=df["description"].str.replace(r'\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s', " ", regex = True)
    df["description"]=df["description"].str.replace(r'\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s', " ", regex = True)
    df["description"]=df["description"].str.replace(r'\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s', " ", regex = True)
    df["description"]=df["description"].str.replace(r'\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]$', " ", regex = True)
    df["description"]=df["description"].str.replace(r'^[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s', " ", regex = True)
    ''''''
    ...
    df["description"]=df["description"].str.replace('(\d+(\.\d+)?)', r' \1 ', regex=True)
    ...
    ## remove more than 1 spaces
    df["description"]=df["description"].str.replace("\s\s+", " ", regex = True)
...
def full_preprocess(df):
    """
    Full preprocessing function that applies all preprocessing steps.
    """
    df = get_investigated_features(df)
    df = price_preprocessing(df)
    df = process_room_type(df)
    df = process_amenities(df)
    df = description_preprocessing(df)
